...
<div class="col-12 p-0 mt-3 " style="font-size: 0.8rem;">
      <?php
                    $var_destiny='';
                    foreach ($destino['breadcrumb'] as $key => $value) {
                        if (!empty($value["url"])) {
                           echo '<a href="'.$value["url"].'" style="color: #333;" class="text-capitalize">'.strtolower($value["txt"]).'</a> > ';
                            ...
                            echo '<span class="text-capitalize pl-1 pr-1 font-weight-bold">'.str_replace("-"," ",$value["txt"]).'</span>';
...
<?php
    // echo "IMAGEN ".json_encode($slider); 
    //echo "<hr>".json_encode($destino)."<hr>";
     echo "
        <script>
        var actividades = ".json_encode($destino).".activity;
...
<div class="envoltura form-group col-md-12">
                ...
                <div class="col-12 imagen" style="padding: 0;height: 250px;overflow: hidden;">
                    <img src="<?=$destino['actual']['slider_img']?>" class="img-fluid" alt="Responsive image" style="width: 100%;height: auto;opacity: 1;position: absolute;left: -100%;right: -100%;top: -100%;bottom: -100%;margin: auto;min-height: 100%;min-width: 100%;">
                  </div>
                <div class="capa">
                </div>
                <div class="path">
                ...
                <div class="destino">
                   <span><?=str_replace("-"," ",$destino['location'])?></span>
                </div>
            </div>
        <div class="container-fluid text-right">
        <?=($language=='es'?'Filtros':'Filters') ?>
            <button class="btn btn-warning btn-sm" style="    position: sticky;
    top: 42px;
    z-index: 2;
    /*border: 0px;*/
    font-size: 20px;
            ...
            <div class="col-12 div-full">
                <div class="row div-full" >
                    ...
                    <div class="col-12" style="margin-right: 0;padding-right: 0;">
                        <div class="row">
                            <div class="row col">
                                <div class="text-left col float-left align-self-center">
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel" data-dismiss="modal"><?=($language=='es'?'Filtros':'Filters') ?></h5>
...
function mostrar_actividades(categoria,duracion,lugar){
    ...
    filtrados.sort(function(a,b){
        if (reset_ordenamiento==0){
            // solo entra una ves
            var a=parseFloat(a.ini_actividad_key);//precio de actividad concadenado con duracion de actividad 1er tipo de duracion, 2do cantidad 
            var b =parseFloat(b.ini_actividad_key);
        }else{
            var a=parseFloat(a.precio_actividad);
            var b =parseFloat(parseInt(b.precio_actividad));
        }
    ...
    filtrados.forEach(function(actividad){
        ...
        tmp_html = tmp_html + `
                            ...
                            <div class="col-md-12  div-activity " id="ip-${actividad.id_producto}">
                                <div class="row content-activity">
                                    <div class="col-md-4 div-img-activity">
                                        <a href="${actividad.url_actividad}#${actividad.id_producto}">
                                            <img class="img-fluid img-activity faded" src="${actividad.url_thumbs}" data-img="${actividad.url_imagen}" alt="">
                                        </a>
                                    </div>
                                    <div class="div-full col-md-8 d-flex div-descripcion-prince">
                                        <div class="row content-descripcion-prince">
                                            <div class="col-12 col-md-9 div-description div-content">
                                                    ...
                                                    <div class="text-left col-md-12 div-card-title">
...
//     console.log('test');
//     console.log($(this).find(".img-activity").data('img'));
//     $(this).find(".img-activity").attr('src', $(this).find(".img-activity").data('img'));
// }, function() {
...
// $(".div-img-activity").css('align-self', 'center');
...
actividades[index].url_thumbs=actividad.url_imagen;
})
    window.addEventListener('load', function() {
      console.log('All assets are loaded');
      // for (var i = 0; i < actividades.length; i++) {
...
if (!empty($lugar)) {
    ...
    echo "if(lugares.indexOf('".$lugar."')>-1){mostrar_actividades(null,null,'".html_entity_decode($lugar)."'); }else{mostrar_actividades(null,null,null);}";
}else{
echo "mostrar_actividades(null,null,null);";
//echo "alert('lugar no exsite');";
}
?>
</script>
