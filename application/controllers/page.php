...
class Page extends CI_Controller {
    ...
    /*
    funcion de index  cargar de golpe el index de incalake.com
    tal y como fue en un inicio no posee algunas mejoras pero funciona
    el problema que surgio con este, es por la calificacion en el pagespeed
    para activa esta funciona solo tienen que renombrar la funcion por index
    asi la reconosca, PERO TIENEN QUE RENOMBRAR LA FUNCION QUE ESTE CON INDEX ANTERIOR
    */
    public function master_index($language = 'es'){
    ...
    /*
    funcion de index retarda la caga del index de incalake.com
    para mejorar la calificacion en el pagespeed
    para activa esta funciona solo tienen que renombrar la funcion por index
    asi la reconosca, PERO TIENEN QUE RENOMBRAR LA FUNCION QUE ESTE CON INDEX ANTERIOR
    */
...
$data['mas_comprados'] = $this->Page_model->get_masComprados($language);
        ...
        //echo json_encode($data['language']).'<br/>';
        //echo json_encode($data['menu_language']);
        $this->load->view("index_custom",$data);
    }
        // search inicio
    ...
    public function view_page($language, $location, $uri_service){
       ...
       // echo $this->uri->segment(2);
        ...
        $temp_ldjson_carrusel_items='';
        $temp_ldjson_carrusel=array();

      

        foreach ($resultado['actividades'] as $key => $value) {
            $temp_galeria=array();
            foreach (@$value['galeria'] as $key2 => $value2) {
                $temp_galeria[]=$value2['url_final'];
            }
            $temp_ldjson_carrusel[] = array(
                'item_url' =>@$resultado['actual']['url_servicio'].'#'.@$value['id_actividad'], 
                'item_name'=>@$value['titulo_producto'],
                'item_image'=>@$temp_galeria,
                'item_description'=>@$value['subtitulo_producto'],
                'item_brand_name'=>@$value['ciudad_cercana'],
                'item_offers_lowPrice'=>'4.4',
                'item_offers_prince'=>$this->oferta_model->get_precio_minimo(@$value['id_actividad']),
            );

        }
        $dato_envio['ldjson_carrusel'] = $temp_ldjson_carrusel;
        if($resultado==false){
