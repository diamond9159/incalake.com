var IncalakeMap = (function() {
    function IncalakeMap(divId) {
        ...
        this.kindOfWaypoints = ['airport', 'busStation', 'hotel', 'houseChanging', 'museum', 'port', 'restaurant', 'ruins', 'sightseeing', 'square', 'trainStation'];
        setTimeout(() => {}, 3000)
    }
    ...
    IncalakeMap.prototype.sortWaypoints = function() {
        ...
        this.waypoints.forEach(function(w, i, a) {
            if (w.tipo >= 0 && w.tipo <= 10) image.url = _this.base_url + "/mapIcons/" + _this.kindOfWaypoints[w.tipo] + ".png";
            else image.url = _this.base_url + "/mapIcons/otro.png";
            ...
            if (!north || north < w.lat) north = w.lat;
            if (!south || south > w.lat) south = w.lat;
            if (!east || east < w.lng) east = w.lng;
            if (!west || west > w.lng) west = w.lng
        });
    ...
    IncalakeMap.prototype.addRouteControl = function() {
        ...
        controlUI.addEventListener('click', function() {
            _this.directionsService.route({
                origin: {
                    lat: _this.waypoints[0].lat,
                    lng: _this.waypoints[0].lng
                },
                destination: {
                    lat: _this.waypoints[_this.waypoints.length - 1].lat,
                    lng: _this.waypoints[_this.waypoints.length - 1].lng
                },
                waypoints: _this.waypoints.filter(function(w, i, a) {
