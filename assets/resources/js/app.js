...
Vue.component('producto-cart', {
  template: `
    <div class="card">
      <div class="card-header bg-success text-light text-center" style="padding:0.45rem;">
          {{cart.producto.titulo_producto}}
      </div>
      <div class="card-body container-fluid rounded-bottom" v-show="!loading" style="background:#1976D2;">
        <div class="row">
          <div class="col-12 col-sm-12" :class="{ 'col-md-5' : !editar }">  <!-- @data {editar} agrega la clase 'col-md-5' segun su valor -->
            <div id="calendario-tour" >
              ...
              <div class="in" class="text-dark form-cart" :id="'content_fecha_tour_'+producto_id" @click="mostrarCalendario=true" style="background:#fff;">  <!-- @click {mostrarCalendario}  cambia el valor al hacer click -->
          ...
          <div class="col-12 col-sm-12" :class="{ 'col-md-5' : !editar }"  @click="mostrarCalendario=false">
            ...
            <div class="col-sm col-md-12" style="padding:0;">
              <div class="row" style="margin:0;padding:3% 0;" v-for="p, index in cart.precios_etapa" v-show="p.visible">
                <div class="text-white col col-sm col-md-4 text-etapa-edad">
                  <strong>{{ p.descripcion_etapa_edad }} {{ p.descripcion_nacionalidad }} </strong> 
                  <span>{{ trans('edad') }} {{ p.edad_minimo }} - {{ p.edad_maximo }}</span>
                </div>
                <div class="col col-sm col-md row text-right p-0" >
                  <div class=" text-white col-12 col-sm-12 col-md text-center" style="padding:1%;">
                    {{ precioPersona(p.precios,index).toFixed(2) }} USD
                  </div>
                  <div class="col-12 col-sm-12 col-md-auto" style="padding:0;">
                    <button @click="cantidadMinima(index)"  class="btn-cantidad bg-success">-</button>
                    <input type="text" v-model="p.cantidad"  class="input-cantidad"/> <!-- v-model valor que puede ser alterado data-binding -->
                    <button @click="cantidadMaxima(index)"  class="btn-cantidad bg-success">+</button>
                    
                  </div>
                </div>
              </div>
              <a href="#" @click="mostrarPreciosPersona($event);" v-show="btnVerMasOpciones" class="text-white text-right more-person">
                <i class="fa fa-chevron-down" aria-hidden="true"></i>  {{ trans('ver_mas_opciones') }}
              </a>
            </div>
            <div class="col-sm col-md text-white" :class="{ 'ammount' : editar }">
              <strong>TOTAL</strong>
              <strong class="float-right" style="font-family: Arial;">
                <del v-if="precio_normal.toFixed(2) != precio_descuento() " style="color:#c37c48;"> {{ precio_normal.toFixed(2) }} </del> 
                &nbsp; {{ precio_descuento() }} USD
              </strong>
              <!--<div style="font-size: 8px;">{{ trans('precio_sin_igv')}}</div>-->
    ...
    store(tipo) {   //Funcion para almacenar el producto a Comprar
      ...
      */
      this.mycart.push({
        ...
        porcentaje_adelanto: this.cart.producto.porcentaje_adelanto, // Porcentaje para calcular el monto a pagar como adelanto 
        pagar_por_adelantado:false,
      });
    ...
    {         
      ...
      { 
        ...
        { 
          ...
          tiempo = ["minutes", "hours", "days"]; // Variables que van a ser utilizadas para calcular la disponibilidad de la reserva en dicho horario.
          fecha_seleccionada = Date.parse(moment(this.fecha_servicio).format('YYYY/MM/DD')+' '+horario); // Convertimos a una fecha con el horario de servicio para validar su disponibilidad
          //console.log("CONVERSION: " + Date.parse(moment(this.fecha_servicio).format('YYYY/MM/DD')+' '+horario));
          //console.log("FECHA SERVICIO222: " + moment(this.fecha_servicio).format('YYYY/MM/DD') + "HORARIO: " +  horario);
          //console.log("FECHA SELECCIONADA: " + fecha_seleccionada);
          fecha_seleccionada = moment(fecha_seleccionada);
          fecha_minima_reserva = fecha_seleccionada.subtract(tiempo_anticipacion[0], tiempo[tiempo_anticipacion[1]]).format('YYYY/MM/DD HH:mm'); //Generamos la fecha minima de reserva
          //console.log("FECHA MINIMA RESERVA1111: " + fecha_minima_reserva + " / " + fecha_seleccionada);
          //console.log( "DIFERENCIA DE FECHAS", moment().format('YYYY/MM/DD HH:mm'), fecha_minima_reserva );
          if(Date.parse(moment().format('YYYY/MM/DD HH:mm')) <= Date.parse(fecha_minima_reserva)) { // CONDICIONAL (fechaActual <= FechaMinimaReserva)
            this.info_horario = "";
...
Vue.component('checkout-carro', {
          ...
          <template v-for="c, index in cart">
            <div class="col-md-12 media div-activity" cart style="margin-bottom: 0px;">
              <div class="row div-full content-activity">
                <div class="  col-md-4 div-img-activity div-full align-self-center">
                  <img class="img-card img-fluid img-activity img-thumbnail" :src="c.img_thumb" alt="" :data-img="c.img" :title="c.titulo_producto" style="width: 100%;">
                  <div class="middle">
                    ...
                    <div class="" style="width: 100%;margin-top:10px">
                      ...
                      <div class="media-labels">
                        <label class="custom-control custom-checkbox mb-2 mr-sm-2 mb-sm-0">
                          ...
                          <span class="custom-control-indicator bg-primary"></span>
  ...
  methods: {
    ...
    eliminarTour(event, index) { //Funcion que es ejecutada para eliminar un producto, recibimos el indice para eliminar ese array especifico
      if (event) event.preventDefault();
      gtagRemoveItemCart(this.cart[index]);
      Vue.delete(this.cart, index);  // Eliminamos el tour
      
      this.cart.forEach((c,index)=>{
        this.cart[index].pagar_por_adelantado=false;
...
Vue.component('resumen-carro', {
  template: `
    ...
    <div class="checkout-sumary mb-2"  style="font-family: Source Sans Pro" socks>
            <div class="subtitle">
            ...
            <template  v-for="c, in cart">
                <div class="item-sumary">
                    <div class="row">
                        <div class="col-8">
                            <div class="">
                                <span v-if="c.porcentaje_adelanto>0&&c.porcentaje_adelanto<100"><!--<span class="text-danger fa fa-asterisk"></span>--> </span> {{ c.titulo_producto }} <br>
                  ...
                  <span v-if="index > 0">,</span> &nbsp;  {{ p.descripcion_etapa_edad }} {{ p.descripcion_nacionalidad }}  x {{ p.cantidad }}
                        ...
                        <div class="col-12" v-if="c.porcentaje_adelanto>0&&c.porcentaje_adelanto<100">
                        <!-- 
                        <div class="form-check tooltip_info" @click="pagominimo(c)" >
                          <input class="form-check-input" type="checkbox" :id="'checkbox_'+c.producto_id">
                          <label class="form-check-label" :for="'checkbox_'+c.producto_id">
                            Remember me
                          </label>
                        </div>
                        -->
<!--
                        <div class="custom-control custom-checkbox mb-2 mr-sm-2 mb-sm-0 " >
                          <span class="" @click="pagominimo(c)" v-model="c.pagar_por_adelantado">
                            <input type="checkbox" class="custom-control-input" :id="'checkbox_'+c.producto_id">
                            <span class="custom-control-indicator" ></span>
                            <label class="custom-control-description text-capitalize" :for="'checkbox_'+c.producto_id">
                                <small>{{ trans('precio_minimo') }}</small> 
                                </label>
                          </span>
                            <span class="fa fa-question-circle text-primary tooltip_info" data-toggle="tooltip"  data-html="true" 
                        v-bind:title="'Activar/Desactivar Precio Minimo o pago de aseguramiento del <span class='+btn_primary+'>'+c.porcentaje_adelanto+'%</span> del tour'"></span>
                        ...
                        <!-- -->
                            <label class="custom-control custom-checkbox mb-2 mr-sm-2 mb-sm-0 " @click="pagominimo(c)" 
                            v-model="c.pagar_por_adelantado"   >
                                <input type="checkbox"  class=" custom-control-input" :id="'checkbox_'+c.producto_id" v-bind:checked="c.pagar_por_adelantado">
                                <span class="custom-control-indicator"></span>
                                <span class="custom-control-description text-capitalize">
                                <small>{{ trans('precio_minimo') }}</small> 
                                </span>
                            </label>
                            <span class="fa fa-question-circle text-primary tooltip_info" data-toggle='tooltip'  data-html='true' v-bind:title="trans('txt_tooltip_precio_minimo')"></span>
                             <!-- -->
                        </div>
                    </div>
                </div>                      
                ...
                <div class="item-sumary">
                    <div class="row">
                        <div class="col-8">
                            <div class="">
                              <span class="text-danger fa fa-asterisk"></span> <strong>{{ trans('tasa_e_impuesto') }} <i title="${langApp == 'es' ? 'Comisiones cobradas por tarjetas de crédito/débito' : 'Fees charged from credit/debit card transactions' }" class="fa fa-question-circle-o text-info"></i> ( {{ tasa_porcentaje() }})</strong> 
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="extra">
                ...
                <div class="item-sumary">
                  <div class="row">
                    <div class="col-md-12">
                      ...
                      <span class="float-right text-secundary">- {{ ( (total()*cupon_valor)/100 ).toFixed(2) }} USD</span>
                      <!--- Modal Cupón -->
            ...
            <div class="total-price text-uppercase col-12">
              <div class="row">
            ...
            <div v-if="this.visible_porcentaje_adelanto">
              <div class="col-12 p-0 pt-2 pb-2 font-weight-bold text-capitalize">{{ trans('modalidad_de_pago') }}</div>
                <div class="col-sm col-md font-weight-bold text-uppercase" style="margin-top: 5px; margin-bottom: 5px;background: #f7f7f79e;">
                 {{ trans('paga_ahora') }}<span class="float-right"><span>{{this.cantidad_porcentaje_adelantado_ahora.toFixed(2)}}</span> USD</span>
                 </div> 
                <div class="col-sm col-md form-group font-weight-bold text-uppercase" style="background: #f7f7f79e;">
                {{ trans('paga_al_llegar') }} <span class="float-right">{{(( total() - ( (total()*cupon_valor)/100 ) )-cantidad_porcentaje_adelantado_ahora).toFixed(2)}} USD</span>
                </div>
            ...
            <div class="col-md-12 mt-2 p-0"  style="font-size:10px;"><span class="text-danger fa fa-asterisk"></span> {{ trans('txt_pago_igv') }}</div>
  ...
  data () {
    return {
      ...
      cantidad_porcentaje_adelanto:0,
      cantidad_porcentaje_adelantado_ahora:0,
      visible_porcentaje_adelanto:false,
      visible_count_porcenta_adelanto:false,
      monto_total_pagar_adelantado:0,
      btn_primary:'" bg-danger text-white font-weight-bold p-1"',
      precio_total:0,
    }
  },
  mounted() {
    data_cookie=JSON.parse(Cookies.get('cart'));
    data_cookie.forEach((value,index)=>{
      this.cantidad_porcentaje_adelanto+=parseFloat(value['porcentaje_adelanto']);
      if (parseFloat(value['porcentaje_adelanto'])!=0&&parseFloat(value['porcentaje_adelanto'])!=100) {
        this.visible_porcentaje_adelanto =true;
      }      
    });
  ...
  methods: {
    ...
    tasa_porcentaje() { //Retorna el porcentaje de la tasa de los productos añadidos al carro.
      ...
      if(isNaN(tasa/this.cart.length)) { //Default tasa
          ...
          return (tasa/this.cart.length).toFixed(2)+' %';  //Visualización de la tasa a cobrar por los servicios de tour seleccionados
      }
    },
    total() { //  Retorna el importe total a pagar de los productos agregados
      // console.log('entrandooo',Date());
      //console.log('---------------------------------INICIO TOTAL---------------------------------');
      //console.log('this.cart',this.cart);
      total_pagar = 0;
      total_pagar_adelantado = 0;
      recurso_precios=0;
      tasa = 0;
      // console.log('wwwwwwwwwwwwwwwwwwwwwww',this.cart);
      this.cart.forEach((c, index) => {
        if (this.cupon_valido) {
          //console.log('Precio Normal');
          tasa += (c.precio_normal * c.tasas_impuestos) / 100;
          total_pagar += c.precio_normal;
          if (c.pagar_por_adelantado) {
            if (parseFloat(c.porcentaje_adelanto)<=0){
              total_pagar_adelantado+=c.precio_normal;
            }else{
              total_pagar_adelantado+=(c.precio_normal*c.porcentaje_adelanto)/100;          
            }
          }else{
            total_pagar_adelantado+=c.precio_normal;
          }
          ...
          total_pagar += c.total;
          // console.log('c.pagar_por_adelantado',c.pagar_por_adelantado);
          if (c.pagar_por_adelantado) {
            if (parseFloat(c.porcentaje_adelanto)<=0){
            total_pagar_adelantado+=c.total;

            }else{
              total_pagar_adelantado+=(c.total*c.porcentaje_adelanto)/100;
              // console.log('porcentaje adelatnado');
            }
            // console.log('entrasss');
          }else{
            total_pagar_adelantado+=c.total;
            // console.log('sin porcentaje adelatnado');
          }
        ...
        if(c.recursos) {
          c.recursos.forEach((r, index) => {
            ...
            total_pagar_adelantado+=r.precio * r.cantidad;
          });
        } 
        //console.log('total_pagar_adelantado',total_pagar_adelantado);
        // console.log('this.cantidad_porcentaje_adelantado_ahora111',total_pagar_adelantado+tasa-this.cupon_valor_descuento);
        // console.log('total_pagar',total_pagar);
        //console.log('(((total_pagar+tasa)*this.cupon_valor)/100',((total_pagar+tasa)*this.cupon_valor)/100);
        if (this.cantidad_porcentaje_adelantado_ahora>0) {
          //console.log('total_pagar_adelantado+tasa-(((total_pagar+tasa)*this.cupon_valor)/100 SI',this.cantidad_porcentaje_adelantado_ahora);
        }else{
          //console.log('total_pagar_adelantado+tasa-(((total_pagar+tasa)*this.cupon_valor)/100 NO',this.cantidad_porcentaje_adelantado_ahora);
            // this.cart[index].pagar_por_adelantado=false;
        }
        
        //console.log('---------------------------------Iteracion total---------------------------------');
      });
      
      //console.log('---------------------------------FIN TOTAL---------------------------------');


      // console.log('antes del clickk',this.cantidad_porcentaje_adelantado_ahora);
      // console.log('this.cupon_valor_descuento',this.cupon_valor_descuento);
      this.tasas_impuestos = tasa; // Tasa cobrada de los diferentes tour
      this.monto_total_pagar_adelantado = total_pagar_adelantado;
      // this.cantidad_porcentaje_adelantado_ahora=total_pagar_adelantado+tasa-( ((total_pagar+tasa)*this.cupon_valor)/100);
      this.cantidad_porcentaje_adelantado_ahora=total_pagar_adelantado+tasa;
      // console.log(this.cantidad_porcentaje_adelantado_ahora+'----'+(total_pagar+tasa));
      if (this.cantidad_porcentaje_adelantado_ahora!=total_pagar+tasa){
        this.visible_porcentaje_adelanto =true;
      }else{
        this.visible_porcentaje_adelanto =false;
      }
      this.precio_total=total_pagar+tasa;
    ...
    operarCupon(){
      if (this.cupon_codigo.trim().length != 0 ) {
        ...
        axios.get(base_url+'api/cupon?codigo='+c_codigo+'&&lang='+langApp).then((response) => {  // APi para consultar cupón
          //this.loading = false;
          console.log('this.cantidad_porcentaje_adelantado_ahora',this.cantidad_porcentaje_adelantado_ahora-( (this.total()*response.data.valor)/100));
          if ( response.data.response === 'success' ) {
            if (this.cantidad_porcentaje_adelantado_ahora-( (this.total()*response.data.valor)/100)>0) {
              console.log('this.cupon_valor_descuento',( (this.total()*this.cupon_valor)/100));
              console.log('this.cantidad_porcentaje_adelantado_ahora222',this.cantidad_porcentaje_adelantado_ahora);
              ...
              alert((langApp=='es'?'este cupon no puede ser aplicado, porque el precio a pagar ahora no supera al monto del cupon de descuento.':'this coupon can not be applied, because the price to pay now does not exceed the amount of the discount coupon.'));
            }
            
          } 
          if (parseFloat(this.cantidad_porcentaje_adelantado_ahora)>parseFloat(this.precio_total-this.cupon_valor)) {
            this.visible_porcentaje_adelanto =false;
            //console.log(this.cantidad_porcentaje_adelantado_ahora+' '+(this.precio_total-this.cupon_valor));
            //console.log('this.visible_porcentaje_adelanto',this.visible_porcentaje_adelanto);
          }else{
            this.visible_porcentaje_adelanto =true;
            //console.log(this.cantidad_porcentaje_adelantado_ahora+' '+(this.precio_total-this.cupon_valor));
          }
    ...
    pagominimo(c){ 
      // console.log('test',c.producto_id);
      //console.log('clickk111',this.cantidad_porcentaje_adelantado_ahora);
      var checkedbox=document.getElementById('checkbox_'+c.producto_id);
      if(Cookies.get('cart')) {
           actividades =  JSON.parse(Cookies.get('cart'));
           actividades.forEach((val,i) => {
              if(val.producto_id == c.producto_id) {
                // if (this.cantidad_porcentaje_adelantado_ahora-this.cupon_valor_descuento>0) {
                  this.cart[i].pagar_por_adelantado=document.getElementById('checkbox_'+c.producto_id).checked;
                  this.total();
                  
                    console.log('checkedbox'+c.producto_id,checkedbox.checked);
                  console.log('this.cantidad_porcentaje_adelantado_ahora-this.cupon_valor_descuento',this.cantidad_porcentaje_adelantado_ahora);
                  if (this.cantidad_porcentaje_adelantado_ahora>0) {
                    ...
                    actividades[i].pagar_por_adelantado=checkedbox.checked;
                    this.cart[i].pagar_por_adelantado=checkedbox.checked;
                  }else{
                    document.getElementById('checkbox_'+c.producto_id).checked=false;
                    this.cart[i].pagar_por_adelantado=document.getElementById('checkbox_'+c.producto_id).checked=false;
                    actividades[i].pagar_por_adelantado=false;
                    alert((langApp=='es'?'esta actividad no puede ser aplicado el pago minimo, porque el monto de descuento supera al precio minimo':'this activity can not be applied the minimum payment, because the discount amount exceeds the minimum price'));
                  }
              }else{
                // console.log('noooooo',val);
              }
           });
           Cookies.remove('cart');
          Cookies.set('cart', actividades,{ expires : expireCookie });
          
          // this.cantidad_porcentaje_adelantado_ahora=123;
        }
        
    //console.log('clickk2222',this.cantidad_porcentaje_adelantado_ahora);
    //console.log('this.cart',this.cart);
    }
  }
});
...
Vue.component('recursos', {
  ...
  template:`
    <div>
      <div class="row recurso mb-1" v-if="recursos.length!=0" v-show="!close">   
        ...
        <div class="col-12 col-md-6" v-for="r, index in recursos">
          <div :class="{gift_selected:r.cantidad!=0}" style="padding-top: 5px;">
              <div class="row" style="margin:0; padding:3px;">
                <div class="col card-img-gif align-self-center" style="vertical-align: top;padding-right: 5px;padding:0;">
                  <img class="img-card d-flex mr-3 img-thumbnail" :src="r.img_thumbs" :data-img="r.img" width="100%" height="50px" :title="r.nombre">
                  <div class="middle">
                    <div class="fa fa-search-plus"></div>
                  </div>
                </div>
                ...
                <div class="col align-self-center" style="padding:0;vertical-align: top;text-align: center">
                  <div>
...
Vue.component('map-incalake', {
  ...
  mounted: function () {
    ...
    var _this = this;
    console.log("==>", _this.id_map, waypoints);
...
Vue.component('customer-form', {
  ...
  mounted() {
    ...
    this.gtagCheckout('begin_checkout'); // Si llega a esta página entonces el cliente ha empezado a navegar en el carro de compras
  ...
  methods: {
    validarFormNext: function () {
      this.$validator.validateAll().then((result) => { //Realiza la validación del formulario
        ...
        this.gtagCheckout('checkout_progress'); // Cuando pasa al formulario de Pagos
          window.location.href = base_url+langApp+'/checkout/payment';
        }
      });
    },
    gtagCheckout(gtagEvent){
        //Inicia Seguimiento a los pasos del carro de compras
        gtagCheckoutProgress(this.cart,gtagEvent);
    }
  },
  watch : { 
...
function gtagAddItemCart(item){
    //console.log(Date());
    var _gtagItem = [];
    (item.personas).forEach((element,index)=>{
        _gtagItem.push( 
            {
                "id": "P"+item.producto_id,
                "name": item.titulo_producto,
                "variant": element.descripcion_etapa_edad,
                "quantity": element.cantidad,
    		    "price": element.precio
            } 
        );
    });
    //console.log(JSON.stringify(_gtagItem));
...
function gtagRemoveItemCart(item){
	var _gtagItem = [];
    (item.personas).forEach((element,index)=>{
        _gtagItem.push( 
            {
    ...
    //console.log(index,JSON.stringify(_object));
}

function gtagCheckoutProgress(_object,_gtagEvent){
    //console.log("_GTAGS",Date());
    //console.log("_GTAGS2",JSON.stringify(_object));
    var _gtagItem = [];
	(_object).forEach((element,index)=>{
        (element.personas).forEach((person, key)=>{
        	_gtagItem.push( 
	            {
	                "id": "P"+element.producto_id,
	                "name": element.titulo_producto,
	                "variant": person.descripcion_etapa_edad,
	                "quantity": person.cantidad,
	    		    "price": person.precio
	            } 
	        );
        });
