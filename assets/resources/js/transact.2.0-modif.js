    ...
    var GTAG_TOTAL = 0, GTAG_TAX = 0; // GTAG VALUES
    ...
    function actualizarDatos(){
        if (Cookies.get('cart')) {
            ...
            console.log("Personas: " + jsonCookie);
            ...
            (jsonCookie).forEach((value,index) => {
                (value['personas']).forEach((val,i) => {
                    ...
                    tempItem = {
                        ...
                        porcentaje_adelanto : value['porcentaje_adelanto']?value['porcentaje_adelanto']:100, // Porcentaje de adelanto para pagar mediante tarjeta.
                        tasas_impuestos: value['tasas_impuestos'],  // Porcentaje para cobrar tasas e impuestos
                        pago_minimo: value['pagar_por_adelantado'], // TRUE = Pagar minimo, Ejemplo: 20%; FALSE = Se cobra todo.
                    };
                ...
                if ( (value['recursos']) ) {
                    (value['recursos']).forEach((v,j) => {
                        tempRecurso = {
                            ...
                            tasas_impuestos: 0, // No hay tasas e impuestos para recursos
        ...
        objPaypal.emailBusiness     = 'lagodelcieloperu@gmail.com';
        //objPaypal.emailBusiness     = 'reservas@incalake.com';
        
        objPaypal.shoppingUrl       = 'http://web.incalake.com/'+langApp+'/checkout/cart';
        
        //objPaypal.returnUrl         = 'https://shop.incalake.com/paypal/success-test.php';
        ...
        GTAG_TOTAL = objItem.montoTotalTours;
        GTAG_TAX = objItem.totalMontoTasasImpuestos;
        
        objCulqi                = new ConfigCulqi();
        //objCulqi.publicKey      = "pk_test_NVbRNu7rdWFpxXI7"; //tEsT
        //objCulqi.publicKey     = 'pk_live_rRgjM0Lbve8ZkrRT';  // iNcAlAkE
        
        objCulqi.publicKey     = 'pk_live_17reIdaSwa03ckDL';    // L.DEL.C.
        //objCulqi.publicKey      = 'pk_test_EbXQf1IgYhjJoCAz';
    ...
    window.registerCustomer = (method) =>  
    {
        ...
        $.ajax({
            ...
            id_reserva = data;
            dataJson.id = data;
            //Data válido solo para paypal
            var dataCustomPaypal = {
                id: data,
                url: document.domain,
                language: document.documentElement.lang,
                type: 'web.incalake.com',
            };
            
            Cookies.set('reserva_incalake', JSON.stringify(dataJson), { domain: '.incalake.com' });
            if(method == 'paypal') {
                document.getElementById("paypalCustomValue").value = JSON.stringify(dataCustomPaypal);
...
/***************************************************************************************/
    ...
    function operarCulqui(){
        ...
        console.log('error up',e.responseText);
      });          
    }
/***************************************************************************************/
    ...
    function culqi() {
        ...
        if(Culqi.token) {
            ...
            var serverData = {
                ...
                subTotalAmount  : (parseFloat(objItem.subTotalMontoPagar)).toFixed(2),
            ...
            $.ajax({
                ...
                console.log("paso ajax 1"); 
                if ( data['response'] === 'success' ) {
                    var jsonCulqi = data['data'];
                    //jsonCulqi['object'] = 'charge';
                    ...
                    if ( jsonCulqi['object'] === 'charge' ) {
                        ...
                        $.post(base_url+'api/reserva/update', { 
                           id_reserva: id_reserva,
                            ...
                            console.log("paso post 2"); 
                            gtagPurchase( JSON.parse(Cookies.get('cart')) , id_reserva );
                            Cookies.remove('cart');
                            Cookies.remove('customer');
                            Cookies.remove('cupon');
                            $.unblockUI();
                            console.log("Intento redirección"); 
                    ...
                    if (data['error_level'] === '1' ) {
                        ...
                        if (data['data']['object'] === 'error' ) {
                            $.unblockUI();
                            alert("error:" + data['data']['user_message']);
...
function gtagCheckoutOption(_payment,_position){
    gtag('event', 'set_checkout_option', {
      "checkout_step": ""+_position,
      "checkout_option": ""+_payment,
      "value": "USD"
    });
}

function gtagPurchase(_object, id ){
    var _gtagItem = [];
