...
Vue.component('resumen-carro', {
  template: `
    ...
    <div class="checkout-sumary"  style="font-family: Source Sans Pro" socks>
                ...
                <div class="item-sumary">
                  ...
                  <span v-if="index > 0">,</span> &nbsp;  {{ p.descripcion_etapa_edad }} {{ p.descripcion_nacionalidad }}  x {{ p.cantidad }}
                        ...
                        <div class="col-12" v-if="c.porcentaje_adelanto>0&&c.porcentaje_adelanto<100">
                            <label class="custom-control custom-checkbox mb-2 mr-sm-2 mb-sm-0" @click="pagominimo($event,cart)">
            ...
            <div class="total-price text-uppercase col-12">
                {{ trans('total_a_pagar') }}  <span class="float-right "> {{ ( total() - ( (total()*cupon_valor)/100 ) ).toFixed(2) }} USD</span>
            </div>
            ...
            <div class="col-12 p-0 pt-2 pb-2 font-weight-bold text-capitalize">{{ trans('modalidad_de_pago') }}</div>
              <div class="col-sm col-md font-weight-bold text-uppercase" style="margin-top: 5px; margin-bottom: 5px;background: #f7f7f79e;">
               {{ trans('paga_ahora') }}<span class="float-right">{{cantidad_porcentaje_adelantado_ahora.toFixed(2)}} USD</span>
               </div> 
              ...
              <div class="col-md-12 form-group"  style="font-size:11px;"><span class="text-danger fa fa-asterisk"></span> {{ trans('txt_pago_adelantado') }}</div>
  ...
  methods: {
    ...
    total() { //  Retorna el importe total a pagar de los productos agregados
      ...
      this.cantidad_porcentaje_adelantado_ahora=total_pagar_adelantado+tasa-this.cupon_valor_descuento;
    ...
    pagominimo(event,index){ 
      if (event) event.preventDefault();
      console.log('test',index);
      if(Cookies.get('cart')) {
           actividades =  JSON.parse(Cookies.get('cart'));
           Cookies.remove('cart');
           actividades.forEach((i,c) => {
              if(c.producto_id == index) {
                actividades[i].pagar_por_adelantado = true;
              }
           });
           actividades.push(this.mycart[0]);
